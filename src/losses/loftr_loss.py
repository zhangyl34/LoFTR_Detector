...
class LoFTRLoss(nn.Module):
    ...
    def compute_coarse_loss(self, conf, conf_gt, weight=None):
        ...
        """
        pos_mask, neg_mask = conf_gt == 1, conf_gt == 0  # (N,HW/64,HW/64)
        c_pos_w, c_neg_w = self.c_pos_w, self.c_neg_w    # 1.0, 1.0
        ...
        elif self.loss_config['coarse_type'] == 'focal':  # this one
            conf = torch.clamp(conf, 1e-6, 1-1e-6)   # 将 conf 约束在一定区间内
            alpha = self.loss_config['focal_alpha']  # 0.25
            gamma = self.loss_config['focal_gamma']  # 2.0
            
            if self.sparse_spvs:  # this one
                pos_conf = conf[:, :-1, :-1][pos_mask] \
                            if self.match_type == 'sinkhorn' \
                            else conf[pos_mask]  # 一维向量
    ...
    def compute_fine_loss(self, expec_f, expec_f_gt):
        if self.fine_type == 'l2_with_std':  # this one
    ...
    def _compute_fine_loss_l2_std(self, expec_f, expec_f_gt):
        ...
        # 'inf' 指无穷范数，即最大值
        correct_mask = torch.linalg.norm(expec_f_gt, ord=float('inf'), dim=1) < self.correct_thr  # < 1.0
    ...
    def forward(self, data):
        ...
        loss_c = self.compute_coarse_loss(
            data['conf_matrix_with_bin'] if self.sparse_spvs and self.match_type == 'sinkhorn' \
                else data['conf_matrix'],  # (N,HW/64,HW/64)
            data['conf_matrix_gt'],
            weight=c_weight)
        loss = loss_c * self.loss_config['coarse_weight']  # * 1.0
        loss_scalars.update({"loss_c": loss_c.clone().detach().cpu()})

        # 2. fine-level loss
        loss_f = self.compute_fine_loss(data['expec_f'], data['expec_f_gt'])  # (vld,3) x,y,std
